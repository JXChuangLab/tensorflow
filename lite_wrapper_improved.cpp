// Copyright 2024 TensorFlow Lite GPU Wrapper - 改进版本
//
...
class ComputeShaderProgram {
 public:
  ...
  absl::Status Initialize(const std::string& shader_source) {
    // 创建计算着色器
    ...
    if (!success) {
      ...
      return absl::InternalError(std::string("计算着色器编译失败: ") + info_log);
    }
    ...
    if (!success) {
      ...
      glDeleteProgram(program_);
      program_ = 0;
      return absl::InternalError(std::string("程序链接失败: ") + info_log);
    }
...
class TfLiteGpuModelImpl {
 ...
 private:
  ...
  absl::Status InitializeGpuRunner(const tflite::FlatBufferModel& model, TfLiteGpuPriority priority) {
    ...
    // 初始化计算着色器
    MP_RETURN_IF_ERROR(InitializeComputeShaders());
  ...
  // 初始化计算着色器
  absl::Status InitializeComputeShaders() {
    // 纹理到SSBO转换着色器源码
    const std::string texture_to_ssbo_source = R"(
#version 310 es
...
void main() {
    ...
    return texture_to_ssbo_shader_.Initialize(texture_to_ssbo_source);
  }
  ...
  absl::Status CopyTextureToSSBO(uint32_t texture_id, uint32_t ssbo_id, int width, int height) {
    ...
    // 设置uniform变量
    texture_to_ssbo_shader_.SetUniform("input_width", width);
    texture_to_ssbo_shader_.SetUniform("input_height", height);
    texture_to_ssbo_shader_.SetUniform("input_channels", 4);
    texture_to_ssbo_shader_.SetUniform("output_width", width);
    texture_to_ssbo_shader_.SetUniform("output_height", height);
    texture_to_ssbo_shader_.SetUniform("output_channels", 4);
    
    // 执行计算着色器
    int work_group_x = (width + 7) / 8;
    int work_group_y = (height + 7) / 8;
 ...
 private:
  ...
  ComputeShaderProgram texture_to_ssbo_shader_;
};
